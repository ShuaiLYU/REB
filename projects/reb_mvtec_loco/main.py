...
parser.add_argument('-coreset' ,type=float ,default=None)
parser.add_argument('-ldknn_factor' ,type=float ,default=None)
parser.add_argument('-K' ,type=int, default=None)
args = parser.parse_args()
if args.coreset:global_param.hyp.coreset=args.coreset
if args.ldknn_factor:global_param.hyp.ldknn_factor=args.ldknn_factor
if args.K: global_param.hyp.ks=[args.K]
#################################################################################
...
if __name__ =="__main__":
    ...
    for cate_idx,cate_name in enumerate(PARAM.all_types):
        ...
        buffer_param = global_param.hyp.clone()
        ...
        # def get_group_data():
        #     datas=[]
        #     for defect_name in PARAM.all_types:
        #         temp_param=PARAM.mvt_data.clone()
        #         temp_param.defect_name=defect_name
        #         datas.append(MvtecLoco(**temp_param, transform=augment_transform))
        #     train_data=DataListWrapper(datas)
        #     train_data.img_size = (args.resize, args.resize)
        #     return train_data
        # train_data=get_group_data()
        ...
        train_eval_data = DatasetWrapper(MvtecLoco(**PARAM.mvt_data), "img", transform=tensor_transform)

        eval_data = DatasetWrapper(MvtecLoco(**PARAM.mvt_data_eval), ("img", "label", "mask"),
        ...
        if PARAM.run_mode in [0,2]:
            if ck.empty():
                ...
                train_data = MvtecLoco(**PARAM.mvt_data,transform=augment_transform)
                # train_data=DatasetWrapper(train_data,new_length=2000)
            ...
            if buffer_param.ad_method=="gde":
                ...
                for  k  in buffer_param.ks:
                    for nn_type in buffer_param.nn_types:
                        ...
                        if nn_type==NN_Type.ldknn: eval_hparam["ld_coefficient"]=buffer_param.ldknn_factor
                        eval_hparam["name"] = "_{}_k{}".format(nn_type, k)
                        eval_inputs.append((eval_hparam,eval_input))

            print(" inputs: number : {}".format(len(eval_inputs)))
