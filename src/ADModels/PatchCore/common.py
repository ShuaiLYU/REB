...
class NearestNeighbourScorer(object):
    ...
    def fit(self, detection_features: List[np.ndarray],reach_field=None,ldn_factor=None) -> None:
    ...
    def predict(
        ...
        if self.reach_field is None:
            anomaly_scores = np.mean(query_distances, axis=-1)
            return anomaly_scores, query_distances, query_nns
        else:

            reach_distance = np.mean(self.reach_field[query_nns].squeeze(), axis=-1,keepdims=True)
            query_distances = query_distances - reach_distance*self.ldn_factor
